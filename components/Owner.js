...
const team = [
  {
    ...
    bio: 'A graduated software engineer turned entrepreneur. Imran brings tech-savvy innovation to the traditional karyana store model, creating a modern, digital-first shopping experience for Bahawalpur.',
    badges: ['🎓 Software Engineer', '💼 Entrepreneur', '🏆 Community Leader'],
    social: { wa: 'https://wa.me/923000000000' },
    isOwner: true,
  },
  {
    name: 'Staff Member',
    ...
    badges: ['🛒 Store Ops', '📦 Inventory'],
    social: { wa: 'https://wa.me/923000000001' },
    isOwner: false,
  },
  {
    name: 'Staff Member',
    role: 'Digital Services Executive',
    number: '+92-300-0000002',
    img: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=500&q=80',
    bio: 'Handles all EasyPaisa, JazzCash, and utility bill transactions with speed and accuracy.',
    badges: ['📱 EasyPaisa', '💚 JazzCash'],
    social: { wa: 'https://wa.me/923000000002' },
    isOwner: false,
  },
]
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-16 reveal">
          ...
          <h2 className="font-display font-black text-4xl sm:text-5xl text-[#083d1e] section-heading mb-4">
            The People Behind<br /><span className="text-[#0f5c2e]">Imran Karyana Store</span>
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {team.map((member, i) => (
            <div key={member.name + i} className={`reveal ${member.isOwner ? 'lg:col-span-1' : ''}`} style={{ transitionDelay: `${i * 100}ms` }}>
              <div className={`card-hover bg-white rounded-3xl overflow-hidden shadow-lg border-2 ${member.isOwner ? 'border-[#c8920a]' : 'border-gray-100'} relative`}>
                {member.isOwner && (
                ...
                <div className="relative h-64 overflow-hidden">
                  <img src={member.img} alt={member.name} className="w-full h-full object-cover object-top transition-transform duration-500 hover:scale-105" loading="lazy" />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#083d1e]/70 to-transparent" />
                  <div className="absolute bottom-4 left-4 right-4">
                    <div className="flex flex-wrap gap-2">
                      {member.badges.map((b, j) => (
                        <span key={j} className="bg-white/20 backdrop-blur-sm text-white text-xs px-2.5 py-1 rounded-full border border-white/30 font-semibold">
                          {b}
                        </span>
                      ))}
                    </div>
                ...
                {/* Info */}
                <div className="p-6">
                  <h3 className="font-display font-black text-2xl text-[#083d1e] mb-1">{member.name}</h3>
                  <div className="flex items-center gap-2 mb-3">
                    <div className="w-2 h-2 rounded-full bg-[#0f5c2e]" />
                    <span className="text-[#0f5c2e] font-bold text-sm">{member.role}</span>
                  </div>
                  <p className="text-gray-500 text-sm font-body leading-relaxed mb-4">{member.bio}</p>

                  {/* Contact */}
                  <div className="border-t border-gray-100 pt-4 flex items-center justify-between">
                    <a href={`tel:${member.number}`} className="flex items-center gap-2 text-[#083d1e] font-semibold text-sm hover:text-[#0f5c2e] transition-colors">
                      <span className="w-7 h-7 rounded-full bg-[#0f5c2e]/10 flex items-center justify-center text-sm">📞</span>
                      {member.number}
                    </a>
                    <a href={member.social.wa} target="_blank" rel="noopener noreferrer"
