    ...
    <footer className="bg-[#051f0f] text-white relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0">
        <svg viewBox="0 0 1440 60" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
          <path d="M0,30 C360,60 1080,0 1440,30 L1440,0 L0,0 Z" fill="#fdf6e3" />
        </svg>
      </div>

      {/* Background decorations */}
      <div className="absolute inset-0 opacity-5"
        style={{ backgroundImage: 'radial-gradient(circle at 10% 80%, #0f5c2e 0%, transparent 50%), radial-gradient(circle at 90% 20%, #c8920a 0%, transparent 50%)' }} />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="pt-20 pb-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <div className="font-display font-black text-xl">Imran Karyana</div>
                <div className="text-[#c8920a] text-xs font-semibold">Store • Bahawalpur</div>
              </div>
            </div>
            <p className="text-white/60 text-sm font-body leading-relaxed mb-5">
              Your trusted neighbourhood karyana store in Bahawalpur. Fresh groceries, digital payments, and daily essentials — all under one roof.
            </p>
            {/* Social Links */}
            <div className="flex gap-3">
              ...
              <a href="mailto:mohammedimranrafique@gmail.com"
          ...
          {/* Quick Links */}
          <div>
            <h4 className="font-display font-bold text-lg mb-5 text-[#f0c040]">Quick Links</h4>
          ...
          {/* Services */}
          <div>
            <h4 className="font-display font-bold text-lg mb-5 text-[#f0c040]">Our Services</h4>
            <ul className="space-y-2">
              {services.map((s, i) => (
                <li key={i} className="text-white/60 text-sm font-body hover:text-white transition-colors">{s}</li>
              ))}
            </ul>
          </div>
          ...
          {/* Contact */}
          <div>
            <h4 className="font-display font-bold text-lg mb-5 text-[#f0c040]">Contact Info</h4>
            <div className="space-y-4">
              {[
                { icon: '📍', label: 'Address', val: 'Bahawalpur, Punjab, Pakistan' },
                { icon: '📞', label: 'Phone', val: '+92-300-0000000' },
                { icon: '📧', label: 'Email', val: 'mohammedimranrafique@gmail.com' },
                { icon: '⏰', label: 'Hours', val: 'Mon–Sun: 8 AM – 10 PM' },
              ].map((item) => (
                <div key={item.label} className="flex items-start gap-3">
                  <span className="text-lg mt-0.5">{item.icon}</span>
                  <div>
                    <div className="text-[#c8920a] text-xs font-bold uppercase tracking-wide">{item.label}</div>
                    <div className="text-white/70 text-sm font-body">{item.val}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-5 bg-[#0f5c2e]/40 border border-[#0f5c2e] rounded-xl p-4">
              <div className="text-[#f0c040] font-bold text-sm mb-1">🚨 Emergency Stock?</div>
              <div className="text-white/70 text-xs">WhatsApp us at any time — we try our best to help!</div>
            </div>
          </div>
        </div>
        ...
        <div className="py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-white/50 text-sm font-body text-center sm:text-left">
            © {year} Imran Karyana Store, Bahawalpur. All rights reserved.
          </div>
          <div className="text-white/50 text-sm font-body flex items-center gap-2">
            Built with <span className="text-red-400 text-base animate-float inline-block">❤️</span> by
