...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      scrolled ? 'bg-white/97 backdrop-blur-md shadow-lg py-2' : 'bg-transparent py-3'
    }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between gap-3">

        {/* Logo */}
        <button onClick={() => handleNav('#home')} className="flex items-center gap-2 group flex-shrink-0">
          <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center font-display font-black text-base sm:text-lg transition-all duration-300 flex-shrink-0 ${
            scrolled ? 'bg-[#0f5c2e] text-[#f0c040]' : 'bg-[#f0c040] text-[#083d1e]'
          } shadow-lg group-hover:scale-110`}>
            IK
          </div>
          <div className="flex flex-col leading-none">
            <span className={`font-display font-bold text-sm sm:text-base transition-colors duration-300 ${scrolled ? 'text-[#0f5c2e]' : 'text-white'}`}>
              Imran Karyana
            </span>
            <span className={`text-xs transition-colors duration-300 ${scrolled ? 'text-[#c8920a]' : 'text-[#f0c040]'}`}>
              Bahawalpur
            </span>
          </div>
        </button>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <a
            ...
            className={`lg:hidden w-9 h-9 rounded-full flex items-center justify-center text-base transition-all duration-300 ${
              scrolled ? 'bg-[#0f5c2e] text-white' : 'bg-white/20 text-white border border-white/30'
            }`}
            aria-label="Call us"
          >
            📞
          ...
          >
            <span className={`block h-0.5 w-5 rounded-full transition-all duration-300 ${scrolled ? 'bg-[#0f5c2e]' : 'bg-white'} ${menuOpen ? 'rotate-45 translate-y-2' : ''}`} />
            <span className={`block h-0.5 w-5 rounded-full transition-all duration-300 ${scrolled ? 'bg-[#0f5c2e]' : 'bg-white'} ${menuOpen ? 'opacity-0 scale-x-0' : ''}`} />
            <span className={`block h-0.5 w-5 rounded-full transition-all duration-300 ${scrolled ? 'bg-[#0f5c2e]' : 'bg-white'} ${menuOpen ? '-rotate-45 -translate-y-2' : ''}`} />
          </button>
        </div>
      </div>
